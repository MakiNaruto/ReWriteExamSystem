...
public class DeleteSubject extends JDialog {
	...
	public DeleteSubject() {
		...
		JLabel label = new JLabel("输入要删除的科目");
		...
		button_1.setBounds(259, 115, 96, 30);
		getContentPane().add(button_1);
		
		JComboBox comboBox = new JComboBox(rk.ReadAll("select * from  学科"));
		comboBox.setBounds(160, 43, 213, 30);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (i==0) {
					sql = new ConnDB();
					SqlStatements = "DELETE FROM 学科 WHERE 学科 = '"+comboBox.getSelectedItem()+"'";
